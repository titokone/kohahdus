...
JavaScript-kieltä ja CSS-tyylitiedostoja. Järjestelmä toimii vuoden 2006 yleisimmillä Firefox ja Internet 
Explorer-selainversioilla.
...
tiedot: etunimi, sukunimi, käyttökieli, kurssi, sähköpostiosoite, opiskelijanumero tai henkilötunnus, sekä
...
Kirjautumissivulla käyttäjä syöttää tunnuksensa, salasanansa ja pudotusvalikoista kurssin nimen ja
käyttökielen. Kurssivalinta määrittää, että minkä kurssin suorituksiin opiskelijan tekemät vastaukset
tallennetaan. Opettajan ollessa kyseessä kurssivalinnalla ei ole merkitystä. Jos kirjautuminen onnistuu,
näytetään tehtävälistaus. Jos kirjautuminen epäonnistuu, käyttäjä palautetaan kirjautumissivulle.
...
tehtävää. Tehtävälle määritellään nimi, aihepiiri, tehtävänanto, tarkastustyyppi (malliratkaisu/kiinteät arvot),
...
\subsection{Opiskelijan tiedot (opettaja)}
...
\subsection{Opiskelijan haku (opettaja)}
...
onko opiskelija yrittänyt jo tehtävän suoritusta, yritysten määrä ja onko yritys onnistunut.
...
mahdollinen valmis koodi mikäli kyseessä on täyttötehtävä, sekä tekstialueet johon
opiskelija kirjoittaa ohjelman ja sille annettavat syötteet. Lisäksi on linkki jolla
pääsee takaisin tehtävälistaukseen.
...
lisäksi näytetään palautetta ratkaisun onnistumisesta/epäonnistumisesta, sekä 
Titokoneen suorituksen lopputila.
...
tehtävätyyppikohtaista tietoa, mutta tälle ei ole näkyvissä tarvetta.
...
muuta taulua. Tänne tallennetaan tehtävien kriteerit ja aihepiirilista.
...
\begin{figure}[h!]
	...
	\caption{Käyttäjätietojen muokkaaminen (huom kaksi erillistä sivulatausta)}
\end{figure}
\newpage
...
\begin{figure}[ht]
	...
	\caption{Tehtävän muokkaaminen (huom kaksi erillistä sivulatausta)}
\end{figure}
\newpage
